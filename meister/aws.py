...
class AWSConnection:
   ...
   def createSecurityGroup(self, name, description):
      ...
      >>> aws = AWSConnection(EC2_ACCESS_ID, EC2_SECRET_KEY)
      >>> aws.createSecurityGroup("name", "description")
      """
      conn.ex_create_security_group(self, name, description)
      return self

   def addSecurityRule(self, name, fromPort, toPort, ip, protocol='tcp'):
      conn.ex_authorize_security_group(name, fromPort, ip, protocol)
      return self
